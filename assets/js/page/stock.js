const App = {
  ...
  methods:{
    ...
    updateData: function(){
      ...
      axios.post('api/update-stock', this.form).then(response => {
        if(response.status == 200){
          this.items = response.data.data
          this.meta = response.data.meta
          this.buttonPage = this.pageButton(this.meta.last_page)
          this.resetForm()
          this.show = false
          notifSuccess('Stock berhasil diupdate')
        }else{
          notifError('Stock gagal diupdate')
        }
      })
      .catch(error => {
