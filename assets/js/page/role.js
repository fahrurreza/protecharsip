const App = {
  ...
  methods:{
    ...
    createData:function(e) {
      ...
      e.preventDefault();
      if(!this.form.role) {
        this.error.role = "type is required";
        this.hasError.role = true;
      } else {
        axios
        .post('api/create-role', this.form)
        .then(response => {
          if(response.status == 200){
            this.items = response.data.data
            this.meta = response.data.meta
    ...
    editData: function(data){
      ...
      this.table.id = data
      this.submit = false
      axios.post('api/show-role', this.table).then(response => {
        if(response.status == 200){
    ...
    updateData: function(data){
      this.form.id = data,
      this.submit = true,
      axios.post('api/update-role', this.form).then(response => {
        if(response.status == 200){
          this.items = response.data.data
          this.meta = response.data.meta
          this.buttonPage = this.pageButton(this.meta.last_page)
    ...
    deleteData: function(data){
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          axios.post('api/delete-role', this.table).then(response => {
              if(response.status == 200){
                this.items = response.data.data
                this.buttonPage = this.pageButton(this.meta.last_page)
                this.resetForm()
