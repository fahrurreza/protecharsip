const App = {
  ...
  methods:{
    ...
    createData:function(e) {
      ...
      this.hasError = [];
      e.preventDefault();
      if(!this.form.rak_name) {
        this.error.rak_name = "type is required";
        this.hasError.rak_name = true;
    ...
    editData: function(data){
      ...
      this.table.id = data
      this.submit = false
      axios.post('api/show-rak', this.table).then(response => {
        if(response.status == 200){
          
          this.form.id = response.data.id
          this.form.rak_name = response.data.rak_name
    ...
    updateData: function(data){
      axios.post('api/update-rak', this.form).then(response => {
        if(response.status == 200){
          this.items = response.data.data
          this.meta = response.data.meta
          this.buttonPage = this.pageButton(this.meta.last_page)
    ...
    deleteData: function(data){
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          axios.post('api/delete-rak', this.table).then(response => {
              if(response.status == 200){
                this.items = response.data.data
                this.buttonPage = this.pageButton(this.meta.last_page)
                this.resetForm()
